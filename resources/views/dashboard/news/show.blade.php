    ...
    <div class="row my-3">
        <div class="col-lg-8">
            <h2 class="mb-3">{{$news->title}}</h2>
                    ...
                    <a href="/dashboard/news/{{ $news->slug }}/edit" class="btn btn-warning mb-2 text-light">
                    ...
                    <form action="/dashboard/news/{{ $news->slug }}" method="post" class="d-inline">
                        @method('delete')
                        @csrf
                        <button class="btn btn-danger mb-2 text-light" onclick="return confirm('Are You Sure?')">
